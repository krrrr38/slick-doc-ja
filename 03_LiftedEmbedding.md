...
[Permalink](http://slick.typesafe.com/doc/1.0.0/lifted-embedding.html "Permalink to Lifted Embedding — Slick 1.0.0 documentation")
...
DDL statements for a table can be created with its ddl method. Multiple DDL objects can be concatenated with %2B%2B to get a compound DDL object which can create and drop all entities in the correct order, even in the presence of cyclic dependencies between tables. The statements are executed with the create and drop methods:

    val ddl = Coffees.ddl %2B%2B Suppliers.ddl
    db withSession {
      ddl.create
      //...
      ddl.drop
...
You can create a query template by calling flatMap on a Parameters object. In many cases this enables you to write a single for comprehension for a template.
...
If you need a custom column type you can implement TypeMapper and TypeMapperDelegate. The most common scenario is mapping an application-specific type to an already supported type in the database. This can be done much simpler by using a MappedTypeMapper which takes care of all the boilerplate:
